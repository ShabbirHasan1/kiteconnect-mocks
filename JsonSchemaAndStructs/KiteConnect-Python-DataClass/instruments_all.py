...
#
#     result = instruments_all_from_dict(json.loads(json_string))
...
class InstrumentsAllElement:
    ...
    @staticmethod
    def from_dict(obj: Any) -> 'InstrumentsAllElement':
        ...
        segment = from_union([Segment, from_none], obj.get("segment"))
        ...
        return InstrumentsAllElement(exchange, exchange_token, expiry, instrument_token, instrument_type, last_price, lot_size, name, segment, strike, tick_size, tradingsymbol)
    ...
    def to_dict(self) -> dict:
        ...
        result["segment"] = from_union([lambda x: to_enum(Segment, x), from_none], self.segment)
...
def instruments_all_from_dict(s: Any) -> List[InstrumentsAllElement]:
    return from_list(InstrumentsAllElement.from_dict, s)


def instruments_all_to_dict(x: List[InstrumentsAllElement]) -> Any:
    return from_list(lambda x: to_class(InstrumentsAllElement, x), x)
