...
mod tests {
    ...
    #[test]
    fn test_mf_orders_json() -> serde_json::Result<()> {
        let jsonfile = crate::utils::read_json_from_file("../mf_orders.json").unwrap();
        let deserialized: MfOrders = serde_json::from_reader(jsonfile)?;
    ...
    #[test]
    fn test_mf_orders_error() -> serde_json::Result<()> {
        let raw_data =
            r#"{"status":"error","message":"Error message","error_type":"GeneralException"}"#;
        let deserialized: MfOrders = serde_json::from_str(raw_data)?;
        // println!("{:#?}", &deserialized);
        assert_eq!(
            deserialized,
            MfOrders {
